      ...
      {posts.map(post => (
        <Card key={post.id} className="p-card space-y-3 break-all lg:space-y-4">
          ...
          <div className="text-secondary-foreground flex flex-wrap items-center gap-x-1.5 gap-y-1 text-xs">
            ...
            <PostCategroyTag key="categories" Icon={Shapes} values={post.categories.map(({ category }) => category)} />
            <PostCategroyTag key="tags" Icon={Tag} values={post.tags.map(({ tag }) => tag)} />
          </div>
...
function PostCategroyTag({ Icon, key, values }: { Icon: LucideIcon; key: PostWhereInputKey; values: { id: string; name: string }[] }) {

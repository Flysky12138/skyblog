...
import { DEFAULT_ORDER_KEY, DEFAULT_SORTORD_KEY, POST_WHERE_INPUT } from '../utils'

const ORDER_KEYS = ['asc', 'desc'] satisfies Prisma.SortOrder[]
const SORTORD_KEYS = ['createdAt', 'updatedAt', 'viewCount'] satisfies (keyof Prisma.PostOrderByWithRelationInput)[]

export const PostSearchParamsSchema = z.strictObject({
  categories: z.string().optional(),
  order: z.enum(ORDER_KEYS).default(DEFAULT_ORDER_KEY),
  page: z.coerce.number().int().positive().default(1),
  sortord: z.enum(SORTORD_KEYS).default(DEFAULT_SORTORD_KEY),

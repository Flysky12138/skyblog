...
export const Live2DProvider = ({ children }: React.PropsWithChildren) => {
  ...
  React.useEffect(() => {
    ...
    EdgeConfigService.get<string>(VERCEL_EDGE_CONFIG_KEY.LIVE2D_SRC)
  ...
  const setMessage = React.useCallback(({ content, priority = 0, timeout = 6000 }: Message) => {
    if (priority < (messageRef.current.priority ?? 0)) {
      return
    }
...
const MessageContent = ({ className }: { className?: string }) => {
...
const Live2DBreakpoint = (props: DisplayByBreakPointProps) => {

...
export const MDXClient = React.memo(({ loadingRender: fallback, source = '' }: MDXClientProps) => {
  ...
  if (error) {
    return <div className="flex h-20 items-center justify-center text-xl">{error.message}</div>
  }
  if (!content) {
    return <div className="flex h-20 items-center justify-center text-xl">无内容</div>
  }

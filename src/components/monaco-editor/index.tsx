...
interface MonacoEditorProps extends LanguageConfig, Omit<SlotProps, 'onChange'>, React.PropsWithChildren {
  asChild?: boolean
  /** 是否为对比模式 */
  isDiffMode?: boolean
...
export const MonacoEditor = ({
  ...
  // 创建编辑器
  const createMonacoEditor = (monaco?: any) => {
    if (!monaco) return
  ...
  React.useEffect(() => {
    ...
    // 外部 value 更新（防止死循环）
    if (!isDiffMode && value != editorRef.current.getValue()) {
      editorRef.current.setValue(value)
    }
  }, [isDiffMode, onChange, value])

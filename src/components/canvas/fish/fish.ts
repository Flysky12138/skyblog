class Fish {
  ...
  private controlStatus() {
    for (let count = this.points.length, i = 0; i < count; i++) this.points[i].updateSelf()
    for (let count = this.points.length, i = 0; i < count; i++) this.points[i].updateNeighbors()
  ...
  private render() {
    ...
    for (let count = this.fishes.length, i = 0; i < count; i++) this.fishes[i].render(this.context)
    this.context.save()
    this.context.globalCompositeOperation = 'xor'
    this.context.beginPath()
    this.context.moveTo(0, this.reverse ? 0 : this.height)
    for (let count = this.points.length, i = 0; i < count; i++) this.points[i].render(this.context)
...
class SURFACE_POINT {
  ...
  private next: null | SURFACE_POINT = null
  private previous: null | SURFACE_POINT = null
  private renderer: Fish

...
// 加载外挂 JS 文件
const loadExternalResource = (url: string) => {
  const el = Object.assign(document.createElement('script'), { async: true, src: url })
...
export default async function Live2D({ path, px, py, view, hitAreaVisible }: Live2DPropsType) {
  try {
    ...
    if (!window.PIXI) {
      await loadExternalResource('/live2d/lib/live2d.min.js')
      await loadExternalResource('/live2d/lib/live2dcubismcore.min.js')
      await loadExternalResource('/live2d/lib/pixi.min.js')
      await loadExternalResource('/live2d/lib/index.min.js')
    }
    // zip 模型文件加载方法实现
    ...
    if (app) {
      ...
      // 显示模型的可交互区域
      model.addChild(Object.assign(new window.PIXI.live2d.HitAreaFrames(), { visible: hitAreaVisible }))
      ...
      app.destroy = (...props: Parameters<PixiApp['destroy']>) => {

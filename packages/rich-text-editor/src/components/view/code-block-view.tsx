  ...
  React.useEffect(() => {
    const onKeyDown = (event: KeyboardEvent) => {
      if ((event.ctrlKey || event.metaKey) && event.key === 'a') {
        ...
        // 检查当前选区是否在这个代码块内
        if ($from.pos >= pos && $from.pos <= pos + nodeSize) {
  ...
  return (
    <NodeViewWrapper className="relative">
      {highlightedHtml ? (
        ...
        <Skeleton>
          <pre className="pointer-events-none select-none" contentEditable={false}>
      ...
      <div className="absolute inset-0">
        <NodeViewContent<'pre'> as="pre" className="my-0 bg-transparent text-transparent caret-foreground" spellCheck="false" />
      </div>
    </NodeViewWrapper>
  )
}

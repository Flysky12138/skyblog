...
export const GET = async (request: NextRequest, { params }: RouteContext<'/api/friends/[id]/cover'>) => {
  ...
  try {
    ...
    if (!id) return Response.json('{id} 值缺失', { status: 400 })

    const friend = await prisma.friend.findUnique({ where: { id } })
    if (!friend) return Response.json('未找到资源', { status: 404 })
    ...
    const uint8Array = await page.screenshot({ type: 'webp' }).then(Buffer.from.bind(Buffer))
